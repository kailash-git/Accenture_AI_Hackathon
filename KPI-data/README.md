...
Detection outputs (weekly anomaly scores, the final flagged-week list) are not stored as parquet
extracts here. They are produced inside the seed pipeline
(`Accenture/Accenture/scripts/generate_mock_data.py`) and land in the `anomalies` table of
`Accenture/Accenture/data/business_bi.db`.
...
Anomaly detection and the flagged-movement list are produced downstream by the seed pipeline
(`Accenture/Accenture/scripts/generate_mock_data.py`), which reads these parquet extracts and writes
its results into `Accenture/Accenture/data/business_bi.db`.
