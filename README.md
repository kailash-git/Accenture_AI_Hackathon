...
Accenture/Accenture/
  ...
  data/                          Committed SQLite database and parquet source extracts (evidence_graph.gpickle
                                   is rebuilt at seed time or at start-up and is excluded from version control)
...
`Accenture/Accenture/data/business_bi.db`) and the graph file is missing, `api_server.py` rebuilds
the graph from the database automatically at start-up. A full reseed is not required.
...
| The start-up banner omits the "evidence graph loaded" line | The graph file is missing and could not be rebuilt | Ensure the committed database is present, or run `python Accenture/Accenture/scripts/build_graph.py` |
